    ...
    <section id="clinica" className="py-32 bg-white relative overflow-hidden below-fold">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-24">
          ...
          <div>
            <Reveal>
              <span className="text-accent-500 font-bold tracking-widest uppercase text-xs mb-4 block">Medicina Veterinária</span>
              <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl text-brand-black leading-tight mb-6">
                Clínica Veterinária <br />
                <span className="text-brand-600">Completa.</span>
              </h2>
            </Reveal>
          ...
          <Reveal delay={100} width="100%">
            <div className="relative rounded-3xl overflow-hidden aspect-4/3 shadow-2xl shadow-brand-900/10">
              <picture>
                <source srcSet="/images/clinica/interior-400.webp 400w, /images/clinica/interior-800.webp 800w" type="image/webp" sizes="(max-width: 1024px) 100vw, 50vw" />
                <img src="/images/clinica/interior.webp" alt="Clínica Veterinária PetPoint" width={1200} height={900} loading="lazy" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
              </picture>
              ...
              <div className="absolute -bottom-4 -right-4 w-40 h-28 rounded-2xl overflow-hidden border-2 border-white shadow-xl hidden md:block">
                <img src="/images/clinica/consulta.webp" alt="Consulta veterinária" width={360} height={240} loading="lazy" className="w-full h-full object-cover" />
              </div>
            </div>
          </Reveal>
        </div>
        ...
        <Reveal delay={200} width="100%">
          <div className="flex items-center justify-between gap-4 mb-10">
            <div className="flex items-center gap-4 flex-1">
              ...
              <span className="text-brand-gray/60 text-xs font-bold uppercase tracking-widest whitespace-nowrap">Nossas especialidades</span>
              <span className="h-px flex-1 bg-brand-200" />
            </div>
            <div className="flex gap-2 shrink-0">
              <button onClick={() => scroll('left')} className="w-8 h-8 rounded-full border border-brand-200 bg-brand-50 hover:bg-brand-100 flex items-center justify-center text-brand-gray transition-all duration-200" aria-label="Anterior">
                <ChevronLeft className="w-4 h-4" />
              </button>
              <button onClick={() => scroll('right')} className="w-8 h-8 rounded-full border border-brand-200 bg-brand-50 hover:bg-brand-100 flex items-center justify-center text-brand-gray transition-all duration-200" aria-label="Próximo">
            ...
            return (
              <div key={idx} className="shrink-0" style={{ width: CARD_W, scrollSnapAlign: 'start' }}>
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-linear-to-t from-brand-900 via-brand-900/50 to-transparent z-30" style={{ opacity: isRevealed ? 0.6 : 0.8, transition: 'opacity 0.5s ease' }} />

                  <div className="absolute bottom-0 left-0 w-full p-6 z-40" style={{ transform: isRevealed ? 'translateY(0)' : 'translateY(6px)', transition: 'transform 0.5s ease' }}>
                    <span className="inline-block px-2 py-1 mb-3 text-[10px] font-bold tracking-widest uppercase bg-accent-500 text-white rounded" style={{ opacity: isRevealed ? 1 : 0, transition: 'opacity 0.3s ease 0.07s' }}>
                      {item.sub}
                    </span>
