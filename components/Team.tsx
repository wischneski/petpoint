  ...
  return (
    <section className="py-32 bg-brand-900 relative overflow-hidden below-fold">
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0 pointer-events-none">
        <div className="absolute top-1/4 -left-64 w-125 h-125 rounded-full" style={{ background: 'radial-gradient(circle, rgba(32,58,143,0.20) 0%, transparent 70%)' }} />
        <div className="absolute bottom-1/4 -right-64 w-125 h-125 rounded-full" style={{ background: 'radial-gradient(circle, rgba(207,46,120,0.10) 0%, transparent 70%)' }} />
      </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12">
          <Reveal>
            ...
            <h2 className="font-serif text-5xl md:text-6xl text-white leading-tight">
              Mãos que cuidam <br />
              <span className="text-transparent bg-clip-text bg-linear-to-r from-brand-400 to-white">com amor e técnica.</span>
            </h2>
          </Reveal>
          ...
          <Reveal delay={200}>
            <div className="flex flex-col items-end gap-6">
              ...
              <div className="flex gap-3">
                <button onClick={() => scroll('left')} className="w-10 h-10 rounded-full border border-white/20 bg-white/5 hover:bg-white/15 flex items-center justify-center text-white transition-all duration-200 hover:border-white/40" aria-label="Anterior">
                  <ChevronLeft className="w-5 h-5" />
                </button>
                <button onClick={() => scroll('right')} className="w-10 h-10 rounded-full border border-white/20 bg-white/5 hover:bg-white/15 flex items-center justify-center text-white transition-all duration-200 hover:border-white/40" aria-label="Próximo">
        ...
        <Reveal delay={150} width="100%">
          ...
          >
            {team.map((member, idx) => {
              ...
              return (
                <div key={idx} className="shrink-0" style={{ width: CARD_WIDTH, scrollSnapAlign: 'start' }}>
                  <div
                  ...
                  >
                    ...
                    >
                      {member.specialty && (
                        <span className="inline-block px-2 py-1 mb-3 text-[10px] font-bold tracking-widest uppercase bg-accent-500 text-white rounded" style={{ opacity: isRevealed ? 1 : 0, transition: 'opacity 0.3s ease 0.1s' }}>
                          {member.specialty}
                        </span>
                      )}
                      <h3 className="text-2xl text-white font-serif mb-1">{member.name}</h3>
                      <p className="text-gray-300 text-sm mb-2">{member.role}</p>
                      {member.crmv && <p className="text-white/40 text-xs mb-6 font-mono">{member.crmv}</p>}

                      <div className="flex gap-4" style={{ opacity: isRevealed ? 1 : 0, transition: 'opacity 0.5s ease 0.2s' }}>
